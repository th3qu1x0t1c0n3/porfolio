        ...
        <div className="row fixed-top mt-5 clearfix bg-dark align-items-center pt-3 pb-1">
            ...
            <div className="col-xl-3 col-lg-3 col-md-3 ms-auto col-sm-11 col-10">
                <div className="row text-light">
                    <a className="col-1 ms-2 " onClick={() => navigate('/portfolio')}>
                        <i className="fa-sharp fa-solid fa-house-user fa-3x text-warning"></i></a>
                    <Form className={`col-6`}>
                        <FormSelect className="form-select badge w-50 bg-light text-dark mt-1 p-2"
                                    aria-label="Default select example" onChange={changeCharacter}
                                    defaultValue={character.id}>
                            {
                                characters.map((character: ICharactere, index) => {
                                    return <option key={index} value={character.id}
                                                   className="fw-bold">{character.name}</option>
                                })
                            }
                        </FormSelect>
                    </Form>
